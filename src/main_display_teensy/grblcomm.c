...
const uint16_t grblStateColor[NUMSTATES] = {
    C_WHITE,      //"Unknown",
    C_WHITE,      //"Idle",
    C_GREEN,      //"Run",
    C_YELLOW,     //"Jog",
...
static void parseData (char *block)
{
    ...
    if(block[0] == '<')
        for (int i = 1;i<strlen(block);i++)
            if (block[i] == '<') {
                serial0_writeLn(grbl_data.block);
                serial0_writeLn(" read error --> discard");               
                return;
            }               
    //serial0_writeLn(grbl_data.block);      // CJ for debugging
...
//    grbl_event.on_line_received = await_ack;
    ...
    timeout_ms += lcd_systicks();         // lcd_systicks() comes from lib interface
